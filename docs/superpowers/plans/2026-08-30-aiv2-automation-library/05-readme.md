...
Read [`00-overview.md`](00-overview.md) first. This file produces the user-facing documentation. [`pyproject.toml`](../../../../pyproject.toml) declares `readme = "README.md"`, so `pip install .` fails until this exists.

**Files:**
- Create: [`README.md`](../../../../README.md)

**Interfaces:**
- Consumes: the finished public API from [`02-aiv2lib-init.md`](02-aiv2lib-init.md).
- Produces: no code. Documents the operator preconditions the library cannot enforce.
...
`0_この れっしゃは ふ….wav` — so consecutive calls would collide. Typing an
...
- [ ] **Step 2: Check every documented name exists**

Run: `python -c "import aiv2lib; print(sorted(n for n in dir(aiv2lib.AIV2Controller) if not n.startswith('_')))"`

Expected: `['assign_voice', 'batch_export_all', 'clear_block', 'focus', 'generate_audio', 'paste_text', 'split_block', 'wait_for_audio']`. The README documents exactly these, and claims `render` does not exist — confirm it does not.

- [ ] **Step 3: Confirm the package builds with the README present**

Run: `python -m pip install --dry-run .`

Expected: metadata resolves without a missing-readme error. If `pip` is unavailable for a dry run, `python -m build --sdist` is an acceptable substitute.

- [ ] **Step 4: Commit**

```bash
git add README.md
git commit -m "docs: document API, preconditions, and completion semantics"
```
